...
function toy_data_load!(path_vae, path_ptvae)
    x = Matrix(CSV.read(string(path_ptvae, "/original_data.csv"), DataFrame, header=false))
    syndata_prior = Matrix(CSV.read(string(path_ptvae, "/vae/prior_sampling/synthetic_data_prior.csv"), DataFrame, header=false))
    syndata_prior_standard = Matrix(CSV.read(string(path_vae, "/vae/prior_sampling/synthetic_data_prior.csv"), DataFrame, header=false))
    x_tr = Matrix(CSV.read(string(path_ptvae, "/pre_transformation/power_transformation/power_transformed_data.csv"), DataFrame, header=false))
    
    return x, syndata_prior, syndata_prior_standard, x_tr
end

function plot_for_PTVAE(path_vae, path_ptvae)

    x, syndata_prior, syndata_prior_standard, x_tr = toy_data_load!(path_vae, path_ptvae)

    # dataList = [x, round_discrete(syndata_prior, x),round_discrete(syndata_prior_standard, x), x_tr]
    dataList = [x, syndata_prior_standard, x_tr, syndata_prior]
    ...
    for i = 1:4
        ...
        for j = 1:3
            if j == 1
                ax[i,j][:plot](data[:,2],data[:,1],  markersize = 1.5, "ob", mew = 0.05,mec="black", mfc="lightblue") # ,
            ...
            elseif j == 2
                ax[i,j][:hist](data[:,2], bins=30, color = "lightblue", edgecolor = "black",linewidth = 0.1)
                ax[i,j][:xaxis][:set_visible](true)
                ...
                ax[i,j][:hist](data[:,1], bins=30, color = "lightblue", edgecolor = "black",linewidth = 0.1)
                ax[i,j][:xaxis][:set_visible](true)
                ax[i,j][:yaxis][:set_visible](false)
...
function bimodality_transform_criteria_plot()
    ...
    for i = 1:4
        ...
        for j = 1:3
            if j == 1
                ax[i,j][:plot](data[:,2],data[:,1],  markersize = 10,linewidths=2, "ob", mec="black", mfc="lightblue") # ,
                ax[i,j][:xaxis][:set_visible](true)
                ax[i,j][:yaxis][:set_visible](true)
                ax[i,j].set(xlim=(floor(minimum(data[:,2])), ceil(maximum(data[:,2]))) ,ylim = (floor(minimum(data[:,1])), ceil(maximum(data[:,1]))), autoscale_on = false)
                ax[i,j].set_yticks(range(floor(minimum(data[:,1])), ceil(maximum(data[:,1])), step = 0.5))
                ...
                ax[i,j].set_ylabel(titleList[i], fontsize=5)
            elseif j == 2
                ax[i,j][:hist](data[:,2], bins=30, color = "lightblue", edgecolor = "black",linewidth = 0.2)
                ax[i,j][:xaxis][:set_visible](true)
                ax[i,j][:yaxis][:set_visible](false)
                ax[i,j].set(xlim=(floor(minimum(data[:,2])), ceil(maximum(data[:,2]))) , autoscale_on = false)
                ...
                ax[i,j][:hist](data[:,1], bins=30, color = "lightblue", edgecolor = "black",linewidth = 0.2)
                ax[i,j][:xaxis][:set_visible](true)
                ax[i,j][:yaxis][:set_visible](false)
                ax[i,j].set(xlim=(floor(minimum(data[:,1])), ceil(maximum(data[:,1]))) , autoscale_on = false)
