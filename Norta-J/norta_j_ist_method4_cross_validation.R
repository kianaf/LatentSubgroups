...
cross_val_dir <- paste0(getwd(), "/Norta-J/run_ist_cross_validation")
...
for (fold in 1:10){
  ...
  url_train <- sprintf("%s/runs/run_457/10fold_cross_validation/fold_%d/train.csv", cur_dir, fold)
  url_test <- sprintf("%s/runs/run_457/10fold_cross_validation/fold_%d/train.csv", cur_dir, fold)

  x_ist <- as.data.frame(read.csv(url_train, header = TRUE))[, 1:15]
  x_ist_test <- as.data.frame(read.csv(url_test, header = TRUE))[, 1:15]
  ...
  for(i in 1:dim(x_ist)[2]){
    ist_min_array[i] <- min(x_ist[,i])
    ist_max_array[i] <- max(x_ist[,i])
    x_ist[,i] <- (x_ist[,i] - ist_min_array[i])/(ist_max_array[i] - ist_min_array[i])
  }
  ...
  colnames(x_ist)<-list_x


  stats <- Return.key.IPD.summaries(Return.IPD.design.matrix(x_ist), 
                                    corrtype = "rank.corr")
  ...
  for(i in 1:dim(x_ist)[2]){
    syn_fed[ ,i] <- (ist_max_array[i] - ist_min_array[i]) * syn_fed[,i] + ist_min_array[i]
  }


  # sample from the created data with the size of the test set

  syn_fed <- syn_fed[sample(1:nrow(syn_fed), nrow(x_ist_test)), ]

  # write the train and test data to the fold directory

  write.csv(x_ist, sprintf("%s/train.csv", write_dir))
  write.csv(x_ist_test, sprintf("%s/val.csv", write_dir))
